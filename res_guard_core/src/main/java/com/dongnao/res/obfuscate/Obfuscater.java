package com.dongnao.res.obfuscate;

import com.dongnao.res.Main;
import com.dongnao.res.arsc.ARSC;
import com.dongnao.res.arsc.StringPoolRef;
import com.dongnao.res.codec.ARSCDecoder;
import com.dongnao.res.utils.FileUtils;
import com.dongnao.res.utils.ZipUtils;
...
public class Obfuscater {
    ...
    private String getTableString(String string) {
        ...
        if (string.startsWith("res")) {
            ...
            if (names != null && names.length == 3) {
                ...
                //混淆后的名字
                newString = newNames[0] + "/" + newNames[1] + "/" + newNames[2] +
                        suffix;
            }
        }
        return newString;
    ...
    private StringPoolRef buildStringPoolRef(StringPoolRef rawStringPoolRef, boolean isTable) {
        try {
            ...
            for (int i = 0; i < strCount; i++) {
                ...
                //写入bytebuffer
                ByteBuffer buffer = ByteBuffer.allocate(offsetLen + strData.length + 1).order
    ...
    public void obfuscate() throws Exception {
        String old_dir_name = Main.PATH + "/app";
...
//        resetCompress(compressData);
        ...
        //arsc临时文件
        File arscFile = new File(Main.PATH + "/temp/resources.arsc");
        ...
        //压缩apk文件
        File outDir = new File(Main.PATH);
        //普通打包
     ...
     * @param dst
     */
    private void sevenZipPacking(File src, File dst) throws Exception {
        //打包、对齐与签名
        /**
         * 打包
         */
        ...
        System.out.println(cmd + " 7z a -tzip " + unsigned_unaligned_apk
                .getAbsolutePath() + " " + src.getAbsolutePath() + "/* -mx=9");
        Process process = Runtime.getRuntime().exec(cmd + " 7z a -tzip " + unsigned_unaligned_apk
                .getAbsolutePath() + " " + src.getAbsolutePath() + "/* -mx=9");
        process.waitFor();
        ...
        process = Runtime.getRuntime().exec(cmd + " zipalign -f 4 " + unsigned_unaligned_apk
                .getAbsolutePath() + " " + unsigned_aligned_apk.getAbsolutePath());
        System.out.println(cmd + " zipalign -f 4 " + unsigned_unaligned_apk
                .getAbsolutePath() + " " + unsigned_aligned_apk.getAbsolutePath());
        process.waitFor();
        if (process.exitValue() != 0) {
...
//        input.apk                                     //被签名的apk
        ...
        process = Runtime.getRuntime().exec(cmd + " apksigner sign --ks " + storeFile
                .getAbsolutePath() + "" +
                ...
                .getAbsolutePath());
        System.out.println(cmd + " apksigner sign --ks " + storeFile
                .getAbsolutePath() + "" +
                " --ks-key-alias androiddebugkey --ks-pass pass:android --key-pass pass:android " +
                "--out " + signed_aligned_apk.getAbsolutePath() + " " + unsigned_aligned_apk
                .getAbsolutePath());
     ...
     * @throws Exception
     */
    private void packing(File src, File dst) throws Exception {
        //打包、对齐与签名
        /**
         * 打包
         */
        ...
        unsigned_aligned_apk.delete();
        Process process = Runtime.getRuntime().exec(cmd + " zipalign -f 4 " + unsigned_unaligned_apk
                .getAbsolutePath() + " " + unsigned_aligned_apk.getAbsolutePath());
        process.waitFor();
        if (process.exitValue() != 0) {
            System.out.println("zipalign error!");
